...
% =========================================================================
function mri = j_mri_process_data_v4(mri)
...
if nb_folders>1
	...
	j_cprintf('Red',['\n!!! Found more than one folder -> merge all volumes together to use motion correction\n'])
...
if ~strcmp(mri.moco_intra.method,'none')
	...
	if isempty(mri.moco_intra.ref)
		...
		fname_data = [mri.nifti.path,mri.nifti.folder{i_folder},mri.nifti.file_data];
		fname_data_firstvols = [mri.nifti.path,mri.nifti.folder{i_folder},mri.nifti.file_data_firstvols];
	...
	fname_data = [mri.nifti.path,mri.nifti.folder{i_folder},mri.nifti.file_data];
	fname_data_firstvols_mean = [mri.nifti.path,mri.nifti.folder{i_folder},mri.nifti.file_data_firstvols_mean];
	fname_data_toreg = [mri.nifti.path,mri.nifti.folder{i_folder},'tmp.data_toreg'];
...
% 	j_progress('Crop data .....................................')
% 	fname_data = [mri.nifti.path,mri.nifti.folder{i_folder},mri.nifti.file_data];
% 	fname_datacrop = [mri.nifti.path,mri.nifti.folder{i_folder},'tmp.data_crop'];
% 	cmd = [fsloutput,'fslroi ',fname_data,' ',fname_datacrop,' ',mri.moco_intra.crop];
% 	[status result] = unix(cmd); % run UNIX command
% 	if status, error(result); end % check error
% 	j_progress(1)
% else
% 	fname_datacrop = [mri.nifti.path,mri.nifti.folder{i_folder},mri.nifti.file_data];
% end
% 
% % crop the ref image
% if ~strcmp(mri.moco_intra.method,'none')
...
% 		fname_data_firstvols_mean = [mri.nifti.path,mri.nifti.folder{i_folder},mri.nifti.file_data_firstvols_mean];
% 		fname_data_firstvols_mean_crop = [mri.nifti.path,mri.nifti.folder{i_folder},'tmp.ref_crop'];
...
if strcmp(mri.moco_intra.method,'none')
	...
	% ... of raw data
	fname_data_merged = [mri.nifti.path,mri.nifti.folder{i_folder},mri.nifti.file_data];
	fname_data_mean = [mri.nifti.path,mri.nifti.folder{i_folder},mri.nifti.file_data,mri.file_suffixe.mean];
	cmd = [fsloutput,'fslmaths ',fname_data_merged,' -Tmean ',fname_data_mean];
	[status result] = unix(cmd);
	if status, error(result); end
	j_progress(0.5)
	fname_data_std = [mri.nifti.path,mri.nifti.folder{i_folder},mri.nifti.file_data,mri.file_suffixe.std];
...
for i_folder=1:nb_folders_moco
	...
	j_cprintf('-black','FOLDER: %s',mri.nifti.folder{i_folder})
	j_cprintf('black','\n')		
	...
	case '2d'
		...
		fname_data = [mri.nifti.path,mri.nifti.folder{i_folder},file_data];
		fname_datasub = [mri.nifti.path,mri.nifti.folder{i_folder},mri.nifti.file_datasub];
		...
		fname_datasub_ref = [mri.nifti.path,mri.nifti.folder{i_folder},mri.nifti.file_datasub_ref];
		...
		for iz=1:mri.nz{1}
			...
			if mri.moco_intra.edge
				fname_datasub = [mri.nifti.path,mri.nifti.folder{i_folder},mri.nifti.file_datasub,num{iz}];
				cmd = [fsloutput,'fslmaths ',fname_datasub,' -edge ',fname_datasub];
				[status result] = unix(cmd); % run UNIX command
				if status, error(result); end % check error
				fname_datasub_ref = [mri.nifti.path,mri.nifti.folder{i_folder},mri.nifti.file_datasub_ref,num{iz}];
				cmd = [fsloutput,'fslmaths ',fname_datasub_ref,' -edge ',fname_datasub_ref];
				[status result] = unix(cmd); % run UNIX command
				if status, error(result); end % check error
			end % edge				
			fname_datasub = [mri.nifti.path,mri.nifti.folder{i_folder},mri.nifti.file_datasub,num{iz}];
			fname_datamoco = [mri.nifti.path,mri.nifti.folder{i_folder},mri.nifti.file_datamoco,num{iz}];
			fname_datasub_ref = [mri.nifti.path,mri.nifti.folder{i_folder},mri.nifti.file_datasub_ref,num{iz}];
		...
		fname_data_moco = [mri.nifti.path,mri.nifti.folder{i_folder},mri.nifti.file_data,mri.nifti.file_data_moco];
		fname_data_moco_3d = [mri.nifti.path,mri.nifti.folder{i_folder},mri.nifti.file_datamoco,'*.nii'];
	...
	case '3d'
		...
		% estimate motion
		fname_data = [mri.nifti.path,mri.nifti.folder{i_folder},file_data];
		fname_datamoco = [mri.nifti.path,mri.nifti.folder{i_folder},mri.nifti.file_data,mri.nifti.file_data_moco];
		fname_data_ref = [mri.nifti.path,mri.nifti.folder{i_folder},mri.nifti.file_data_firstvols_mean];
	...
	case 'afni'
		...
		fname_data = [mri.nifti.path,mri.nifti.folder{i_folder},file_data,'.nii'];
		fname_datamoco = [mri.nifti.path,mri.nifti.folder{i_folder},'tmp.data_moco.nii'];
...
% 		j_progress(1)
	...
	fname_datamoco = [mri.nifti.path,mri.nifti.folder{i_folder},'tmp.data_moco.nii'];
	fname_datamoco_final = [mri.nifti.path,mri.nifti.folder{i_folder},mri.nifti.file_data,mri.nifti.file_data_moco,'.nii'];
	cmd = [fsloutput,'fslroi ',fname_datamoco,' ',fname_datamoco_final,' 1 ',num2str(mri.nt{i_folder})];
	disp(['>> "',cmd,'"'])
	j_progress('Remove the first image of the series (target) .')
	[status result] = unix(cmd); % run UNIX command
	if status, error(result); end % check error
	j_progress(1)
	...
	j_progress('Create Mean and SD image (for quality check) ..')
	% ... of moco data
	fname_data_merged = [mri.nifti.path,mri.nifti.folder{i_folder},mri.nifti.file_data,mri.nifti.file_data_moco];
	fname_data_mean = [mri.nifti.path,mri.nifti.folder{i_folder},mri.nifti.file_data,mri.nifti.file_data_moco,mri.file_suffixe.mean];
	cmd = [fsloutput,'fslmaths ',fname_data_merged,' -Tmean ',fname_data_mean];
	[status result] = unix(cmd);
	if status, error(result); end
	j_progress(0.25)
	fname_data_std = [mri.nifti.path,mri.nifti.folder{i_folder},mri.nifti.file_data,mri.nifti.file_data_moco,mri.file_suffixe.std];
	cmd = [fsloutput,'fslmaths ',fname_data_merged,' -Tstd ',fname_data_std];
	[status result] = unix(cmd);
	if status, error(result); end
	j_progress(0.5)
