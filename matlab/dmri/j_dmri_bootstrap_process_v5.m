...
% 2009-10-30: created
% 2012-02-26: assume bootstrap data don't have b=0 image.
% 2012-03-15: does't open mask, use j_disp()
% =========================================================================
function bootstrap = j_dmri_bootstrap_process_v5(bootstrap)


% Parameters
sh_order = [0 6 0 15 0 28 0 45 0 0 0 91 0 0 0 153]; % nb of coefficients given the max order of SH decomposition

if isfield(bootstrap,'fname_log'), fname_log = bootstrap.fname_log; else fname_log = 'log_j_dmri_bootstrap_process_v5.txt'; end
...
% START FUNCTION
j_disp(fname_log,['\n\n\n=========================================================================================================='])
j_disp(fname_log,['   Running: j_dmri_bootstrap_process_v5'])
j_disp(fname_log,['=========================================================================================================='])
...
if bootstrap.process_dti
	...
	j_disp(fname_log,['\nEstimate DTI on generated data ................'])
...
% flip bvecs for visu
j_disp(fname_log,['\nFlip bvecs for visualisation purpose...'])
bvecs_temp = bvecs;
...
bvecs = bvecs_temp;
clear bvecs_temp
j_disp(fname_log,['.. flip is set to: [',num2str(bootstrap.visu.flip(1)),' ',num2str(bootstrap.visu.flip(2)),' ',num2str(bootstrap.visu.flip(3)),']'])
...
j_disp(fname_log,['.. null bvecs found: ',num2str(index_NullBvecs)])

% load mask
j_disp(fname_log,['\nLoad cropped mask...'])
mask = logical(read_avw([bootstrap.nifti.path,bootstrap.file_mask_crop]));
% mask = reshape(mask,bootstrap.nx,bootstrap.ny);
% mask_new = reorient(mask,bootstrap.orientation);
% j_progress(1)
disp(['.. ',pwd,filesep,bootstrap.file_mask_crop,])

% Find zeros-values on the data (to speed-up things)
j_disp(fname_log,['\nFind zeros-values on the data...'])
data3d = read_avw([bootstrap.fname_data{1}]);
% mask = logical(ones(bootstrap.nx,bootstrap.ny));
icount=1;
...
j_disp(fname_log,['.. Number of bootstraps: ',num2str(bootstrap.nb_bootstraps)])
j_disp(fname_log,['.. Total number of voxels to process: ',num2str(length(find(mask)))])
j_disp(fname_log,['.. Method for Q-Ball estimation: "',bootstrap.qball.method,'"'])
...
gfa_new = zeros(bootstrap.nb_bootstraps,size(mask,1),size(mask,2));
qball_new = zeros(bootstrap.nb_bootstraps,size(mask,1),size(mask,2),sh_order(bootstrap.qball.order_odf));
...
j_disp(fname_log,['\nEstimate Q-Ball on bootstrap data...'])
...
case 'max'
	...
	for i_bootstrap = 1:bootstrap.nb_bootstraps
		...
		% load bootstrap data
		data3d = read_avw([bootstrap.fname_data{i_bootstrap}]);
% 		mask2d = logical(reshape(mask,bootstrap.nx*bootstrap.ny,1));
% 		data2d = reshape(data3d,bootstrap.nx*bootstrap.ny,size(data3d,3));
% 		data2d_masked = data2d(mask2d,:);
% 		clear data2d data3d
% 		nb_voxe
