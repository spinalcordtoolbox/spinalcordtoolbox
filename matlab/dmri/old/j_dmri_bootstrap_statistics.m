...
% =========================================================================
function bootstrap = j_dmri_bootstrap_statistics(bootstrap)


% bootstrap.file_jsd_std		= 'bootstrap_jsd_std.nii';

% open DTK file for the 181 mesh scheme
% fname_odf_scheme = [dmri.dtk.folder_mat,dmri.dtk.file_odf_scheme];
% fid = fopen(fname_odf_scheme,'r');
% odf_scheme = textread(fname_odf_scheme,'','delimiter',',');
...
% compile MEX files
j_progress('Generate MEX files ............................')
path_to_j_dmri_getMaxODF_mex = which('j_dmri_getMaxODF_mex.c');
...
j_progress('Find each sample''s neighbour on the sphere ....')
vert = scheme.vert;
...
disp(['-> Number of samples on the sphere: ',num2str(bootstrap.sampling_visu)])
disp(['-> Number of neighbors: ',num2str(bootstrap.nb_neighbours)])
...
% 			if bootstrap.display_stuff, j_progress(1); end
			...
			use_mex = 1;
			if use_mex
				[max_odf_mean nb_maxima_mean_vox] = j_dmri_getMaxODF_mex(squeeze(odf_mean(ix,iy,:))',scheme.vert,nearest_neighb,double(bootstrap.nb_neighbours),double(bootstrap.maxima_angular_threshold),double(bootstrap.maxima_amplitude_threshold));
			else
			...
			for ib = 1:nb_bootstraps
				if use_mex
					[max_odf{ib} nb_maxima_vox(ib)] = j_dmri_getMaxODF_mex(odf(ib,:),scheme.vert,nearest_neighb,double(bootstrap.nb_neighbours),double(bootstrap.maxima_angular_threshold),double(bootstrap.maxima_amplitude_threshold));
				else
...
% 			figure
	...
	% 				% Now compute the angular error for the 2nd direction, but only
	% 				% if this one exists! And check whether the 2nd direction
	% 				% is closer to the 1st one
	% 				if size(max_odf_mean,1) >= 2
	% 					if size(max_odf{ib},1) >= 2
	% 						angular_error_tmp2(1) = abs(acos(max_odf_mean(2,:)*max_odf{ib}(1,:)'))*180/pi;
	% 						angular_error_tmp2(2) = abs(acos(max_odf_mean(2,:)*max_odf{ib}(2,:)'))*180/pi;					
	% % 						angular_error_tmp2 = abs(acos(max_odf_mean(1,:)*max_odf{ib}(2,:)'))*180/pi;
	% % 						angular_error_tmp2 = abs(acos(max_odf_mean(2,:)*max_odf{ib}(2,:)'))*180/pi;
	% 						if angular_error_tmp2(1) > bootstrap.angular_threshold
	% 							angular_error_tmp2(1) = abs(acos(max_odf_mean(2,:)*(-max_odf{ib}(1,:)')))*180/pi;
	% 						end
	% 						if angular_error_tmp2(2) > bootstrap.angular_threshold
	% 							angular_error_tmp2(2) = abs(acos(max_odf_mean(2,:)*(-max_odf{ib}(2,:)')))*180/pi;
	% 						end
	% 						angular_error_tmp(2) = abs(acos(max_odf_mean(1,:)*max_odf{ib}(2,:)'))*180/pi;
	% 						if angular_error_tmp(2) > bootstrap.angular_threshold
	% 							angular_error_tmp(2) = abs(acos(max_odf_mean(1,:)*(-max_odf{ib}(2,:)')))*180/pi;
	% 						end
	% 						% get the minimum of cumulative errors across the two directions
	% 						if angular_error_tmp(1)+angular_error_tmp2(2) < angular_error_tmp(2)+angular_error_tmp2(1)
	% 							angular_error(ib) = angular_error_tmp(1);
	% 							angular_error2(ib2) = angular_error_tmp2(2);
	% 						else
	% 							angular_error(ib) = angular_error_tmp(2);
	% 							angular_error2(ib2) = angular_error_tmp2(1);
	% 						end
	% 						ib2 = ib2+1;
	% 					end
	% 				end
	...
	% 				line([0 max_odf{ib}(1,1)],[0 max_odf{ib}(1,2)],[0 max_odf{ib}(1,3)])
	% 				hold on
