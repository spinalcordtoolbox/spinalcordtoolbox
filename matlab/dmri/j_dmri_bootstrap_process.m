...
% =========================================================================
function bootstrap = j_dmri_bootstrap_process(bootstrap)
...
bvecs = bvecs(bootstrap.nifti.nb_b0+1:end,:);
fclose(fid);
j_progress(1)

% flip bvecs for visu
j_progress('Flip bvecs for vizualisation purpose ..........')
bvecs_temp = bvecs;
...
mask = logical(j_mri_read([bootstrap.nifti.path,bootstrap.nifti.file_mask_crop]));
...
data3d = j_mri_read([bootstrap.fname_data{1},'.nii']);
icount=1;
...
case 'max'
	...
	for i_bootstrap = 1:bootstrap.nb_bootstraps
		...
		% load bootstrap data
		data3d = j_mri_read([bootstrap.fname_data{i_bootstrap},'.nii']);
