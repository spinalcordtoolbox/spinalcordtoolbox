...
% 2012-03-23: use bvecs instead of bvals to identify nb of directions
% =========================================================================
function bootstrap = j_dmri_bootstrap_generate_v4(bootstrap)



% INITIALIZATION
if isfield(bootstrap,'outputtype'), outputtype = bootstrap.outputtype; else outputtype = 'NIFTI'; end
if isfield(bootstrap,'fname_log'), fname_log = bootstrap.fname_log, else fname_log = 'log_j_dmri_bootstrap_generate.txt'; end
...
% START FUNCTION
j_disp(fname_log,['\n\n\n=========================================================================================================='])
j_disp(fname_log,['   Running: j_dmri_bootstrap_generate_v4'])
j_disp(fname_log,['=========================================================================================================='])
...
j_disp(fname_log,['.. Orientation is: ',orientation])
...
j_disp(fname_log,['.. Cropping coordinates are: ',num2str(coord_roi)])

% Crop data
j_disp(fname_log,['\nNumber of folders is (in case of repetition bootstrap): ',num2str(bootstrap.nb_folders)])
if bootstrap.crop
	j_disp(fname_log,['\nCrop original data...'])
...
fname_mask_crop = [bootstrap.nifti.path,bootstrap.file_mask_crop];
cmd = [fsloutput,'fslroi ',fname_mask,' ',fname_mask_crop,' ',coord_roi];
unix(cmd);
j_disp(fname_log,['.. ',fname_mask_crop])

% get the dimensions
j_disp(fname_log,['\nGet number of volumes in each set...'])
bvecs = textread([bootstrap.nifti.path,bootstrap.nifti.folder{1},bootstrap.nifti.file_bvecs]);
nt = size(bvecs,1);
j_progress(1)
j_disp(fname_log,['.. Number of volumes is: ',num2str(nt)])

% create write folder
j_disp(fname_log,['\nCreate write folder...'])
path_write = [bootstrap.path];
if ~exist(path_write), mkdir(path_write), end
j_disp(fname_log,['.. Output folder is: ',path_write])
...
case 'bootknife'
	...
	j_disp(fname_log,['\nBootstrap data using "repetition bootknife"'])
...
case 'residual'
	...
	j_disp(fname_log,['\nBootstrap data using "residual bootstrap"'])
...
% 	j_disp(opt.fname_log,['\nDefine sampling for SH order...'])
	...
	for i_bootstrap = 1:nb_bootstraps
        ...
        % display progress
        j_disp(fname_log,['.. ',num2str(i_bootstrap),'/',num2str(nb_bootstraps)])
	end	
end
