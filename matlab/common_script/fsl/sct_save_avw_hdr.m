function sct_save_avw_hdr(img,fname,vtype,vsize)
% Create and save an analyse header file for either a 2D or 3D or 4D array (automatically determined). Based on sct_save_avw_hdr.m from FSL. 
% 
...
% swap first and second argument in case save_avw_img convention is used
...
tmp=sprintf('FSLOUTPUTTYPE=NIFTI_PAIR; export FSLOUTPUTTYPE; $FSLDIR/bin/fslcreatehd %d %d %d %d %6.4f %6.4f %6.4f %6.4f 0 0 0 %d %s',dims(1),dims(2),dims(3),dims(4),vsize(1),vsize(2),vsize(3),vsize(4),dtype,fname);
call_fsl(tmp);
disp(' ');
