function sct_save_avw_complex(img,fname,vsize)
% Create and save an analyse header (.hdr) and image (.img) file. Based on save_avw_complex.m from FSL.
% 
...
sct_save_avw(real(img),[fname,'R'],'f',vsize);
sct_save_avw(imag(img),[fname,'I'],'f',vsize);
