...
[basename, path, ext]=sct_tool_remove_extension(img_fname,1);
if nargin<2
...
for level=1:size(t2.img,3);
    ...
    [initialArray(:,:,level), SC(:,:,level)] = myelinInitialSegmention(255*(double(t2.img(:,:,level))/double(max(max(t2.img(:,:,level))))), BW, logical(zeros(t2.dims(2:3))));
end
