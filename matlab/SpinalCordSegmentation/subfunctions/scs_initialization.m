...
if strcmp(input(end-3:end),'.mat') == 0
    ...
    % Search the pertutation matrix
    if orientation{1} == 'L', m_permute(1) = 1; elseif orientation{1} == 'R', m_permute(1) = 1; end
    if orientation{1} == 'A', m_permute(2) = 1; elseif orientation{1} == 'P', m_permute(2) = 1; end
    if orientation{1} == 'I', m_permute(3) = 1; elseif orientation{1} == 'S', m_permute(3) = 1; end
    
    if orientation{2} == 'A', m_permute(2) = 2; elseif orientation{2} == 'P', m_permute(2) = 2; end
    if orientation{2} == 'L', m_permute(1) = 2; elseif orientation{2} == 'R', m_permute(1) = 2; end
    if orientation{2} == 'I', m_permute(3) = 2; elseif orientation{2} == 'S', m_permute(3) = 2; end
    
    if orientation{3} == 'I', m_permute(3) = 3; elseif orientation{3} == 'S', m_permute(3) = 3; end
    if orientation{3} == 'A', m_permute(2) = 3; elseif orientation{3} == 'P', m_permute(2) = 3; end
    if orientation{3} == 'L', m_permute(1) = 3; elseif orientation{3} == 'R', m_permute(1) = 3; end
    
    m_nifti=permute(m_nifti, m_permute);
    dims([1 2 3]) = dims(m_permute);
    scales([1 2 3]) = scales(m_permute);
    
    % Define the orientation standard based on template image (MNI152)
    if orientation{1} == 'L', m_nifti = flipdim(m_nifti,2); orientation{1} = 'R'; end
    if orientation{1} == 'A', m_nifti = flipdim(m_nifti,1); orientation{1} = 'P'; end
    if orientation{1} == 'I', m_nifti = flipdim(m_nifti,3); orientation{1} = 'S'; end
    
    if orientation{2} == 'A', m_nifti = flipdim(m_nifti,1); orientation{2} = 'P'; end
    if orientation{2} == 'L', m_nifti = flipdim(m_nifti,2); orientation{2} = 'R'; end
    if orientation{2} == 'I', m_nifti = flipdim(m_nifti,3); orientation{2} = 'S'; end
    
    if orientation{3} == 'I', m_nifti = flipdim(m_nifti,3); orientation{3} = 'S'; end
    if orientation{3} == 'A', m_nifti = flipdim(m_nifti,1); orientation{3} = 'P'; end
    if orientation{3} == 'L', m_nifti = flipdim(m_nifti,2); orientation{3} = 'R'; end
    
    % Save final orientation and permutation
    param.orient_new = cell2mat(orientation);
    param.permute = m_permute;
    
    % Repermute to run the code
    m_nifti=permute(m_nifti, [2 1 3]);
    dims([1 2 3]) = dims([2 1 3]);
...
while strcmp(info{1},'no')
    global Slice;
    scs_get_slice(m_nifti, 'Set minimum value of the spinal cord');
    slice_min = Slice;
    scs_get_slice(m_nifti, 'Set maximum value of the spinal cord');
    slice_max = Slice;
    clear global Slice;
    slice_min=round(slice_min); slice_max = round(slice_max);
    param.slices(2) = max(slice_min,slice_max);
    param.slices(1) = min(slice_min,slice_max);
    m_volume=m_nifti;
    m_volume(:,:,param.slices(2)+1:dims(3))=[];   % Suppression of the last slices
    m_volume(:,:,1:param.slices(1))=[];         % Suppression of the first slices
    clear info
    info{1}=questdlg('Are you satisfied with your choice?','Confirmation','yes','no','yes');
    if isempty(info{1})
        info{1}='yes';
    end
end
