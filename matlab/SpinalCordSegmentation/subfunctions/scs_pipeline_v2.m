...
%  input{1,2} = [path_name_in '02_anat_cadotte_2013_05_29'];              % File containing the Center line coordinates for the automatic initialization
...
for i=1:size(input,1)
    ...
    param{i}.update_multiplier = 1.3;   % Multiplies the force applied to deform the radius
    param{i}.shear_force_multiplier = 0.5;  % Multiplies the shear force used to stay near the user defined center line. 
    param{i}.max_coeff_horizontal = 5; % Maximal Fourier coefficients used to smooth the radius in the horizontal plane. Default: 10. The more, the less coefficient, the smoother the curviline.
    param{i}.max_coeff_vertical = 5;   % Maximal coefficient used to smooth the radius in the vertical direction (depends on the number of slices)
    ...
    param{i}.file_type = input{i,1}(end-2:end);   % Gets the file extension from the input
    
    param{i}.nbIterMax = 30;            % Maximal number of iterations for each coefficients (if there is no convergence)
...
%% Straigthening of the image
 ...
 load(output{1,4});
 m_straight_permut = permute(m_straight, [3 2 1]);
scs_slider_3dmatrix(m_straight_permut, 'Visualization of the spinal cord straightened');

%% Saving the centerline and the segmented cord surface in NIfTI
scs_nifti_save_v3(input{1,1},output{1,1},output{1,2},output{1,3},output{1,4},param);
