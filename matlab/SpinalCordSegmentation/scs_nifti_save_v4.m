function [] = scs_nifti_save_v4(input1,output1,output2,output3,output_straightnened,param)
% scs_nifti_save
...
for i=1:size(centerline,3)
    m_surface(:,round(cl_last_iter(3,i)),:) = poly2mask(contour(:,2,i),contour(:,1,i),size(m_surface,1),size(m_surface,3));
end

m_surface = double(m_surface);

% Save of the surface in a NIfTI image
save_avw_v2(m_surface, output3, 'f', scales(1:3));
