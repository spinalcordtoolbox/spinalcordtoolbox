...
URL_ANTS="https://dl.dropboxusercontent.com/u/20592661/spinalcordtoolbox/ants_debian.tar.gz"
...
if [ "$(whoami)" == "root" ]; then
  echo "Sorry, you are root. Please type the following command without sudo: ./install_ants_debian.sh"
  echo
  exit
fi
...
cmd="tar -zxvf ants_debian.tar.gz"
echo ">> $cmd"; $cmd

# move folder
echo
echo "Move folder to: "
cmd="sudo mv ants ${PATH_INSTALL}"
echo ">> $cmd"; $cmd
