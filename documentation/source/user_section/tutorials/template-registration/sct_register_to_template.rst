...
After spinal cord labeling, we can register the subject with the template. e have the labeled spinal cord, we can register the anatomical image to the template. SCT provides the ``sct_register_to_tempalte`` command for template registration. Here are the steps for the algorithm within this command:
