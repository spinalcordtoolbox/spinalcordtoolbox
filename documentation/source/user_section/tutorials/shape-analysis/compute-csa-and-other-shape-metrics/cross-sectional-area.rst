...
First, we will start by computing the cord cross-sectional area (CSA) averaged across vertebral levels. As an example, we'll choose the C2 and C3 vertebral levels, but you can specify any levels that are covered by the appropriate inferior/superior disc labels present in your disc label file.
...
:Input arguments:
   ...
   - ``-vert`` : The vertebral levels to compute metrics across. Vertebral levels can be specified individually (``2,3``) or as a range (``2:3``).
   - ``-discfile`` : Disc label file used to map slices to vertebral levels. Here, we use labels generated by :ref:`sct_deepseg_spine`.
...
.. csv-table:: CSA values computed for C2 and C3 vertebral levels (Averaged)
   :file: csa_c2c3.csv
   :header-rows: 1
...
.. csv-table:: CSA values computed for C2 and C3 vertebral levels
...
:Input arguments:
   ...
   - ``-discfile`` : Disc label file used to report vertebral-level correspondence for each slice.
   - ``-o`` : The output CSV file.
...
.. csv-table:: CSA values computed at 60 mm from the PMJ.
   :file: csa_pmj.csv
   :header-rows: 1
