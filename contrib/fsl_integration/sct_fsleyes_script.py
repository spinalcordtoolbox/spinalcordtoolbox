...
#    - overlayList
#
# These objects are generated by FSLeyes during initialization, and relate to the current
# state of the GUI. A good description of these objects can be found on the following page:
#    - https://open.win.ox.ac.uk/pages/fsl/fsleyes/fsleyes/userdoc/fsleyes_notebook.html
#      (Even though this page is about Jupyter Notebooks, the descriptions are still useful.)
#
# Additionally, this script makes use of the <fsl.data.image.Image> class to create new
# image objects. This class is provided by the FSLeyes context. For more information:
#    - https://open.win.ox.ac.uk/pages/fsl/fslpy/fsl.data.image.html#fsl.data.image.Image
#
# To avoid writing "# noqa: F821" comments everywhere, we "define" them here:

frame = frame              # noqa: F821 (provided by the FSLeyes context)
displayCtx = displayCtx    # noqa: F821 (provided by the FSLeyes context)
overlayList = overlayList  # noqa: F821 (provided by the FSLeyes context)
Image = Image              # noqa: F821 (provided by the FSLeyes context)
...
def run_main():
    ...
    (frame.auiManager.GetPane(sct_view_panel)
                     .Bottom()
