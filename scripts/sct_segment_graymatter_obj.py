...
class SegmentGrayMatter():
    def __init__(self, fname_ref, fname_moving, moving_name, fixed_name, fname_output, moving_seg_name='', fixed_seg_name=''):
        self.fname_ref = fname_ref
        self.fname_moving = fname_moving
        self.fname_output = fname_output
        self.moving_name = moving_name
        self.fixed_name = fixed_name
        self.moving_seg_name = moving_seg_name
        self.fixed_seg_name = fixed_seg_name
        self.path_output, self.file_output, self.ext_output = sct.extract_fname(fname_output)
        self.check_file_exist()

    def check_file_exist(self):
        sct.check_file_exist(self.fname_ref)
        sct.check_file_exist(self.fname_moving)
        if (self.fname_seg_moving != '' and self.fname_seg_fixed == '') or\
                (self.fname_seg_moving == '' and self.fname_seg_fixed != ''):
                print('\nERROR: You need to provide one mask for each image (moving and fixed)')
                usage()
        if self.fname_seg_moving != '':
            sct.check_file_exist(self.fname_moving)
        if self.fname_seg_fixed != '':
            sct.check_file_exist(self.fname_seg_fixed)

    def copy_file(self, path_tmp):
        print('\nCopy files...')
        sct.run("sct_c3d "+self.fname_moving+" -o "+path_tmp+"/"+self.moving_name+".nii")
        sct.run("sct_c3d "+self.fname_ref+" -o "+path_tmp+"/"+self.fixed_name+".nii")
        if self.fname_seg_moving != '':
            sct.run("sct_c3d "+self.fname_moving+" -o "+path_tmp+"/"+self.moving_seg_name+".nii")
        if self.fname_seg_fixed != '':
            sct.run("sct_c3d "+self.fname_moving+" -o "+path_tmp+"/"+self.fixed_seg_name+".nii")

    def crop_xy(self):
        fixed_name_temp = fixed_name + "_crop"
    cmd = "sct_crop_image -i " + fixed_name + ".nii -o " + fixed_name_temp + ".nii -m ../" + fname_seg_fixed + " -shift 10,10 -dim 0,1"
    print cmd
    sct.run(cmd)
