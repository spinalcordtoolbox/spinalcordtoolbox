...
class ParamSeg:
    def __init__(self):
        ...
        self.thr_similarity = (
            0.0005
        )  # epsilon but on normalized to 1 similarities (by slice of dic and slice of target)
...
class SegmentGM:
    def __init__(self, param_seg=None, param_model=None, param_data=None, param=None):
        ...
        # create model:
        self.model = Model(
            param_model=self.param_model, param_data=self.param_data, param=self.param
        )
        ...
        self.info_preprocessing = (
            None
        )  # dic containing {'orientation': 'xxx', 'im_sc_seg_rpi': im, 'interpolated_images': [list of im = interpolated image data per slice]}

        self.projected_target = (
            None
        )  # list of coordinates of the target slices in the model reduced space
    ...
    def segment(self):
        ...
        printv(
            "\nProject target image into the model reduced space...",
            self.param.verbose,
            "normal",
        )
        self.project_target()
        ...
        printv(
            "\nLabel fusion of model slices most similar to target slices...",
            self.param.verbose,
            "normal",
        )
        ...
        if (self.param_seg.path_results != "./") and (
            not os.path.exists(os.path.join(curdir, self.param_seg.path_results))
        ):
        ...
        self.fname_res_gmseg = os.path.join(
            self.param_seg.path_results,
            add_suffix("".join(extract_fname(self.param_seg.fname_im)[1:]), "_gmseg"),
        )
        self.fname_res_wmseg = os.path.join(
            self.param_seg.path_results,
            add_suffix("".join(extract_fname(self.param_seg.fname_im)[1:]), "_wmseg"),
        )
    ...
    def register_target(self):
        ...
        im_dest = self.get_im_from_list(
            np.array([self.model.mean_image for target_slice in self.target_im])
        )
        ...
        shutil.move(
            os.path.join(path_warping_fields, fname_src2dest),
            os.path.join(path_warping_fields, fname_src2dest_save),
        )
        shutil.move(
            os.path.join(path_warping_fields, fname_dest2src),
        ...
        for i, target_coord in enumerate(self.projected_target):
            ...
            for j, dic_coord in enumerate(self.model.fitted_data):
                ...
                if self.param_seg.fname_level is not None:
                    ...
                    similarity = np.exp(
                        -self.param_seg.weight_level
                        * abs(self.target_im[i].level - self.model.slices[j].level)
                    ) * np.exp(-self.param_seg.weight_coord * square_norm)
                else:
            ...
            list_norm_similarities = [
                float(s) / sum(list_dic_similarities) for s in list_dic_similarities
            ]
    ...
    def label_fusion(self, list_dic_indexes_by_slice):
        for target_slice in self.target_im:
            ...
            list_dic_slices = [
                self.model.slices[j] for j in list_dic_indexes_by_slice[target_slice.id]
            ]
    ...
    def warp_back_seg(self, path_warp):
        ...
        im_src_gm = self.get_im_from_list(
            np.array([target_slice.gm_seg_M for target_slice in self.target_im])
        )
        #
    ...
    def post_processing(self):
        ...
        nx_ref, ny_ref, nz_ref, nt_ref, px_ref, py_ref, pz_ref, pt_ref = (
            im_sc_seg_original_rpi.dim
        )
        ...
        printv(
            "  Interpolate result back into original space...",
            self.param.verbose,
            "normal",
        )

        for iz, im_iz_preprocessed in enumerate(
            self.info_preprocessing["interpolated_images"]
        ):
            ...
            ] = im_sc_seg_original_rpi.transfo_pix2phys(
            ...
            sq_size_pix = int(
                self.param_data.square_size_size_mm / self.param_data.axial_res
            )
            [
                [x_square_center_phys, y_square_center_phys, z_square_center_phys]
            ] = im_res_slice.transfo_pix2phys(
            ...
            im_res_slice.hdr.as_analyze_map()["qoffset_x"] += (
                x_center_phys - x_square_center_phys
            )
            im_res_slice.hdr.as_analyze_map()["qoffset_y"] += (
                y_center_phys - y_square_center_phys
            )
            ...
            if len(im_res_slice_interp.data.shape) == 3:
                ...
                im_res_slice_interp.data = im_res_slice_interp.data.reshape(
                    (shape_x, shape_y)
                )
        ...
        printv(
            "  Reorient resulting segmentations to native orientation...",
            self.param.verbose,
            "normal",
        )

        im_res_gmseg.save("res_gmseg_rpi.nii.gz").change_orientation(
            self.info_preprocessing["orientation"]
        ).save("res_gmseg.nii.gz", mutable=True)

        im_res_wmseg.save("res_wmseg_rpi.nii.gz").change_orientation(
            self.info_preprocessing["orientation"]
    ...
    def validation(self):
        ...
        sct_maths.main(
            args=["-i", fname_seg, "-sub", fname_manual_gmseg, "-o", fname_manual_wmseg]
        )

        # compute DC:
        try:
            status_gm, output_gm = run(
            ...
            sct_register_multimodal.main(
                args=["-i", fname_manual_gmseg, "-d", fname_gmseg, "-identity", "1"]
            )
            sct_maths.main(
                args=[
                    "-i",
            ...
            sct_register_multimodal.main(
                args=["-i", fname_manual_wmseg, "-d", fname_wmseg, "-identity", "1"]
            )
            sct_maths.main(
                args=[
                    "-i",
        ...
        fname_dc = (
            "dice_coefficient_" + extract_fname(self.param_seg.fname_im)[1] + ".txt"
        )
        ...
        # compute HD and MD:
        fname_hd = (
            "hausdorff_dist_" + extract_fname(self.param_seg.fname_im)[1] + ".txt"
        )
        run(
        ...
        os.chdir(curdir)
        sct.copy(
            os.path.join(self.tmp_dir, tmp_dir_val, fname_dc),
            self.param_seg.path_results,
        )
        sct.copy(
            os.path.join(self.tmp_dir, tmp_dir_val, fname_hd),
            self.param_seg.path_results,
        )
...
def main(args=None):
    ...
    seg_gm = SegmentGM(
        param_seg=param_seg, param_data=param_data, param_model=param_model, param=param
    )
    seg_gm.segment()
    ...
    if param_seg.qc is not None:
        generate_qc(
            param_seg.fname_im_original,
            seg_gm.fname_res_gmseg,
    ...
    sct.display_viewer_syntax(
        [param_seg.fname_im_original, seg_gm.fname_res_gmseg, seg_gm.fname_res_wmseg],
