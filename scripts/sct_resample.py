...
def resample():
    ...
    # resample data
    if len(n) == 3:
        data_r = resample(nii, transform=transfo, reference=nii_r, mov_voxel_coords=True, ref_voxel_coords=True, dtype='double', interp_order=interp_order)
    elif len(n) == 4:
        data_r = np.zeros(n_r)
        ...
        for it in range(n[3]):
            ...
            # resample data
            data3d_r = resample(nii3d, transform=transfo, reference=nii_r3d, mov_voxel_coords=True, ref_voxel_coords=True, dtype='double', interp_order=interp_order)
