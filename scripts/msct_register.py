...
def register_slicewise(fname_src,
    ...
    shutil.move(warp_inverse_out, os.pardir)
    shutil.move(warp_forward_out, os.pardir)
...
def register2d_centermassrot(fname_src,
    ...
    for iz in range(0, nz):
        ...
        # if one of the slice is empty, ignore it
            ...
            sct.printv('WARNING: Slice #' + str(iz) +
    ...
    if not poly == 0 and rot == 1:
        ...
        if verbose == 2:
            ...
            plt.plot(
                180 * angle_src_dest_regularized / np.pi, 'r', linewidth=2)
            plt.grid()
            plt.xlabel('z')
    ...
    for iz in z_nonzero:
        ...
        centermass_dest_phy = im_src.transfo_pix2phys(
            [[centermass_dest[iz, :].T[0], centermass_dest[iz, :].T[1], iz]])[
                0]
        ...
        coord_inverse_phy = np.array(
            np.dot((coord_init_phy - np.transpose(centermass_src_phy)), R3d.T)
        ...
        if verbose == 2 and not angle_src_dest[iz] == 0:
            ...
            for isub in [221, 222, 223, 224]:
                ...
                plt.text(
                    ...
                    verticalalignment='bottom')
                plt.plot(
                    [0, pcaaxis[0, 0]], [0, pcaaxis[1, 0]],
                    linewidth=2,
                    color='red')
                plt.plot(
                    [0, pcaaxis[0, 1]], [0, pcaaxis[1, 1]],
                    linewidth=2,
                    color='orange')
                plt.axis([-3, 3, -3, 3])
...
def register2d_columnwise(fname_src,
    ...
    for iz in range(0, nz):
        ...
        if np.any(src1d > th_nonzero) and np.any(dest1d > th_nonzero):
            ...
            Sx = (
                dest1d_max - dest1d_min + 1) / float(src1d_max - src1d_min + 1)
            # apply transformation to coordinates
            coord_src2d_scaleX = np.copy(
                coord_src2d)  # need to use np.copy to avoid copying pointer
            coord_src2d_scaleX[:, 0] = (coord_src2d[:, 0] - mean_src_x
                                        ) * Sx + mean_dest_x
            coord_init_pix_scaleX = np.copy(coord_init_pix)
            coord_init_pix_scaleX[:, 0] = (coord_init_pix[:, 0] - mean_src_x
                                           ) * Sx + mean_dest_x
            ...
            for ix in xrange(nx):
                ...
                if np.any(src1d > th_nonzero) and np.any(dest1d > th_nonzero):
                    ...
                    Sy = (dest1d_max - dest1d_min + 1
                          ) / float(src1d_max - src1d_min + 1)
                    ...
                    coord_init_pix_scaleY[range_x, 1] = (
                        coord_init_pix[range_x, 1] - mean_src_y
                    ) * Sy + mean_dest_y
                    coord_init_pix_scaleYinv[range_x, 1] = (
                        coord_init_pix[range_x, 1] - mean_dest_y
                    ) / float(Sy) + mean_src_y
            # apply transformation to image
            col_scaleYinv = np.reshape(coord_init_pix_scaleYinv[:, 1],
                                       [nx, ny])
            src2d_scaleXY = warp(
                src2d, np.array([row_scaleXinv, col_scaleYinv]), order=1)
            ...
            if verbose == 2:
                ...
                ax = plt.subplot(141)
                plt.imshow(
                    np.swapaxes(src2d, 1, 0),
                    cmap=plt.cm.gray,
                    interpolation='none')
                plt.hold(True)  # add other layer
                plt.imshow(
                    np.swapaxes(dest2d, 1, 0),
                    cmap=plt.cm.copper,
                    interpolation='none',
                    alpha=0.5)
                plt.title('src')
                ...
                ax = plt.subplot(144)
                plt.imshow(
                    np.swapaxes(src2d_scaleXYsmooth, 1, 0),
                    cmap=plt.cm.gray,
                    interpolation='none')
                plt.hold(True)  # add other layer
                plt.imshow(
                ...
                ax.grid(True, color='w')
                # save figure
                plt.savefig(path_qc + 'register2d_columnwise_image_z' + str(iz)
                            + '.png')
                plt.close()
...
def register2d(fname_src,
    ...
    for i in range(nz):
        # set masking
        sct.printv('Registering slice ' + str(i) + '/' + str(nz - 1) + '...',
                   verbose)
        num = numerotation(i)
        ...
        cmd = (
            ...
            '--metric ' + paramreg.metric + '[dest_Z' + num + '.nii' + ',src_Z'
            + num + '.nii' + ',1,' + metricSize +
        ...
        try:
            ...
            if paramreg.algo in ['Rigid', 'Affine']:
                ...
                sct.run(
                    'isct_antsRegistration -d 2 -t SyN[1, 1, 1] -c 0 -m MI[dest_Z'
                    + num + '.nii, src_Z' + num +
                    '.nii, 1, 32] -o warp2d_null -f 1 -s 0')
                ...
                sct.run('isct_ComposeMultiTransform 2 ' + file_warp2d +
                        ' -R dest_Z' + num + '.nii warp2d_null0Warp.nii.gz ' +
                        file_mat)
                sct.run('isct_ComposeMultiTransform 2 ' + file_warp2d_inv +
                        ' -R src_Z' + num +
                        '.nii warp2d_null0InverseWarp.nii.gz -i ' + file_mat)
    ...
    if paramreg.algo in ['Translation']:
        ...
        generate_warping_field(
            'dest.nii', x_disp_a, y_disp_a,
            fname_warp=fname_warp)  # name_warp= 'step'+str(paramreg.step)
