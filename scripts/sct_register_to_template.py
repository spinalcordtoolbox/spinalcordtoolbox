...
def main():
    ...
    for i_step in range(1, len(paramreg.steps)+1):
        ...
        sct.run('sct_apply_transfo -i '+src+' -d '+dest+' -w '+','.join(warp_forward)+' -o '+add_suffix(src, '_regStep'+str(i_step))+' -x '+interp_step, verbose)
        src = add_suffix(src, '_regStep'+str(i_step))
        # register src --> dest
