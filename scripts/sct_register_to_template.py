...
def get_parser():
    ...
    parser.usage.set_description('Register an anatomical image to the spinal cord MRI template (default: PAM50).\n\n'
                                 ...
                                 'If two labels are provided, a linear transformation (translation + rotation + superior-inferior linear scaling) will be applied. The strategy here is to defined labels that cover the region of interest. For example, if you are interested in studying C2 to C6 levels, then provide one label at C2 and another at C6. Note that if the two labels are close (e.g. C3 and C5) the vertebral alignment is acccurate. However, if the two labels are very far apart (e.g. C2 and T12), there might be mis alignment of the labels in-between because a subject''s vertebrae might not match exactly the vertebrae of the template.\n\n'
                                 'If more than two labels (only with the parameter "-disc") are used, a non-linear registration will be applied to align the each intervertebral disc between the subject and the template, as described in sct_straighten_spinalcord. This this the most accurate and preferred method. This feature does not work with the parameter "-ref subject".\n\n'
