...
def unzip(compressed, dest_folder, verbose):
    ...
    if compressed.endswith("zip"):
        ...
        try:
            ...
            sct.printv(
                "ERROR: ZIP package corrupted. Please try again.", verbose, "error"
            )
    else:
        sct.printv(
            "ERROR: The file %s is of wrong format" % compressed, verbose, "error"
        )
    ...
    for url in urls:
        try:
            ...
            if "Content-Disposition" in response.headers:
                ...
                sct.printv(
                    "Unexpected: link doesn't provide a filename", type="warning"
                )
                continue
            ...
            with open(tmp_path, "wb") as tmp_file:
                ...
                tqdm_bar = tqdm(
                    total=total, unit="B", unit_scale=True, desc="Status", ascii=True
                )
