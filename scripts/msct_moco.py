...
def moco(param):
    ...
    shutil.copy(file_target + ext, 'target.nii')
    file_target = 'target'
    ...
    for indice_index in range(nt):
        ...
        # run 3D registration
        failed_transfo[it] = register(param, file_data_splitT_num[it], file_target, file_mat[it],
        ...
        if param.iterative_averaging and indice_index < 10 and failed_transfo[it] == 0:
            ffile_target = file_target + ext
            cmd = '-i %s -mul %d -o %s' % (ffile_target, (indice_index + 1), ffile_target)
            sct_maths.main(cmd.split())
            cmd = '-i %s -add %s%s -o %s' % (ffile_target, file_data_splitT_moco_num[it], ext, ffile_target)
            sct_maths.main(cmd.split())
            cmd = '-i %s -div %d -o %s' % (ffile_target, (indice_index + 2), ffile_target)
    ...
    for it in fT:
        abs_dist = [abs(gT[i] - it) for i in range(len(gT))]
        if not abs_dist == []:
            index_good = abs_dist.index(min(abs_dist))
            sct.printv('  transfo #' + str(it) + ' --> use transfo #' + str(gT[index_good]), verbose)
            # copy transformation
            os.copy(file_mat[gT[index_good]] + 'Warp.nii.gz', file_mat[it] + 'Warp.nii.gz')
            # apply transformation
            cmd = '-i %s.nii -d %s.nii -w %sWarp.nii.gz -o %s.nii -x %s' % (
                file_data_splitT_num[it], file_target, file_mat[it], file_data_splitT_moco_num[it], param.interp)
    ...
    try:
        ...
        sct.printv('Warning: can not delete target.nii ' + err, verbose, 'warning')
...
def register(param, file_src, file_dest, file_mat, file_out):
    ...
    if param.todo == 'apply':
        cmd = '-i %s.nii -d %s.nii -w %sWarp.nii.gz -o %s.nii -x %s' % (file_src, file_dest, file_mat, file_out,
                                                                        param.interp)
        sct_apply_transfo.main(cmd.split())

    # check if output file exists
    if not os.path.isfile(file_out + '.nii'):
        sct.printv('WARNING in ' + os.path.basename(__file__) + ': Improper calculation of mutual information. '
                   'Either the mask you provided is too small, or the subject moved a lot. '
                   'If you see too many messages like this try with a bigger mask. '
                   'Using previous transformation for this volume.', param.verbose, 'warning')
        failed_transfo = 1
...
def spline(folder_mat, nt, nz, verbose, index_b0=[], graph=0):
    ...
    if not os.path.exists(old_mat):
        os.makedirs(old_mat)
    for text_file in glob.glob(folder_mat + '*.txt'):
        shutil.copy(text_file, old_mat)
