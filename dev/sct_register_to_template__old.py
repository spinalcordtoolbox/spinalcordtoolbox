...
# TODO: !!! crop the warping field after padding!!!! cannot do it directly with c3d because it's a composite image:
...
def main():
   ...
   # Apply transformation: template --> anat
    ...
    if fname_anat_mask != '':
        print '\nReslice: mask --> source...'
        cmd = 'c3d '+fname_src+' '+fname_anat_mask+' -reslice-identity tmp.mask2src.nii.gz'
        print('>> '+cmd)
        status, output = commands.getstatusoutput(cmd)
        if debugging:
            print output
    ...
    cmd = 'c3d '+fname_src+' -pad 0x0x'+str(padding)+'vox 0x0x'+str(padding)+'vox 0 -o tmp.src_padded.nii.gz'
    print(">> "+cmd)
    status, output = commands.getstatusoutput(cmd)
