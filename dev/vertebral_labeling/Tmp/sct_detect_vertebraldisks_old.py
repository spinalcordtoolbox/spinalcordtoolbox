...
def labeling_vertebrae_T1(param,anat,hdr,dims,scales):
    verbose          = param.verbose

    #==================================================
    # Calculation of the profile intensity
    #==================================================
    sct.printv('\nCalculation of Profile Intensity...',verbose)
...
#=======================================================================================================================
def labeling_vertebrae_T2(param,anat,hdr,dims,scales):
    verbose          = param.verbose
    ...
    #using iir filter for detrending intensity
    Wn = np.amax(frequency)/10
    N = 5                      #Order of the filter
    b, a = scipy.signal.iirfilter(N, Wn, rp=None, rs=None, btype='low', analog=False, ftype='bessel', output='ba')
    I_fit = scipy.signal.filtfilt(b, a, I[:,0], axis=-1, padtype='constant', padlen=None)
...
#    from scipy.interpolate import UnivariateSpline
#    fit = UnivariateSpline(loc_peak_I,value_I.T)
#    P = fit(np.arange(len(I_detrend)))
...
#    corr_peak  = np.nan
    ...
    #=====================================================================
    sct.printv('\nBuilding of Labeled Centerline and Surface...',verbose)
