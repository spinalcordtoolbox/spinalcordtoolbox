...
vector<int> SymmetricalCropping::symmetryDetectionFull(int dimension)
{
    // We assume the orientation to be AIL.
    ...
    for (int init_slice=0; init_slice<desiredSize[1]; init_slice++)
    {
        ...
        middle_slices.push_back(mutualInformation.begin()->second);
    }
    middleSlice_ = 0;
    for (int i=0; i<middle_slices.size(); i++)
        middleSlice_ += middle_slices[i];
    middleSlice_ /= middle_slices.size();
...
int SymmetricalCropping::symmetryDetection(int dimension)
{
    ...
    if (initSlice_ == -1.0) {
        ...
        init_slice = desiredSize[1]*initSlice_;
    }
...
int SymmetricalCropping::symmetryDetectionAP()
{
    // We assume the orientation to be AIL.
...
ImageType::Pointer SymmetricalCropping::cropping()
{
    ...
    /*typedef itk::ImageFileWriter< ImageType > Writer3DType;
    Writer3DType::Pointer writer = Writer3DType::New();
    itk::NiftiImageIO::Pointer io = itk::NiftiImageIO::New();
    writer->SetImageIO(io);
    writer->SetFileName("/home/django/benjamindeleener/data/PropSeg_data/t2star/errsm_01/test.nii.gz");
    writer->SetInput(outputImage_);
    try {
        writer->Update();
    } catch( itk::ExceptionObject & e ) {
