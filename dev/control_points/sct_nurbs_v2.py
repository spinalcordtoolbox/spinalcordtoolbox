...
class NURBS():
    def __init__(self, degre=3, precision=1000, liste=None, sens=False, nurbs_ctl_points=None, size=None, div=None):
        """
        ...
        if sens:                  #### si on donne les points de controle#####
            ...
            if nurbs_ctl_points == None and size == None:
                self.nbControle = len(P_z)/5  ## ordre 3 -> len(P_z)/10, 4 -> len/7, 5-> len/5
                # permet d'obtenir une bonne approximation sans trop "interpoler" la courbe
                #   increase nbeControle if "short data"
            elif (size and div) != None:
                self.nbControle = size/div
            elif nurbs_ctl_points != None:
