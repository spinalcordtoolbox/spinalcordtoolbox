...
def returnSquaredErrors(fname = '', div = 0):
	if fname == '':
	...
	size = splinap.getSize(x, y, z, p1, p2, p3)


	txt = open('results.txt','a')
	txt.write(str(div)+' '+str(int(round(size))/div)+' '+str(size)+' '+str(msx)+' '+str(msy)+' '+' '+str(mean_ms)+' '+fname+'\n')
	txt.close
