...
void Mesh::setReferential(const Referential& ref, bool local)
{
	...
	if (repereLocalbool) {
		CMatrix4x4 transformation = repereLocal_.getTransformation(ref);
		for (unsigned int i=0; i<points_.size(); i++)
			points_[i]->setPosition(transformation*points_[i]->getPosition());
	}
	repereLocal_ = ref;
	repereLocalbool = true;
	calculateLocalPoints(repereLocal_.getTransformation());
	cout << " Done" << endl;
}


void Mesh::calculateLocalPoints(CMatrix4x4 transformation)
{
	if (pointsLocal_.size() > 0) {
		...
		for (unsigned int i=0; i<points_.size(); i++)
		{
			...
			newPoint->setPosition(transformation*newPoint->getPosition());
...
void Mesh::save(string filename)
{
	...
	string fileN = filename + "_mesh.vtk";
	writer->SetFileName(fileN.c_str());
...
void Mesh::decimation(float nb)
{
	...
	if (nb >= 1.0)
		...
		ratio = nb;
	cout << "Demarage de la decimation. Cible : " << nb/3 << " Ratio : " << ratio << endl;
	int label = getLabel();
