    ...
    for jj=1:dims(2)
        for kk=1:dims(3)
            ...
            if roi(ii,jj,kk) && b1Map(ii, jj, kk)~=0
                ...
                ci = confint(fitresult,0.95); % confidence interval for the fitting
                ci(isnan(ci))=0;
                
                linear_regression = coeffvalues(fitresult); % slope and intercept
                linear_regression(isnan(linear_regression))=0;

                [M0(ii,jj,kk),T1(ii,jj,kk)]=getM0T1(linear_regression,TR);
