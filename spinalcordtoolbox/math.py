...
def restore_detail(warped, raw, mask, sigmas=(0.5, 0.5), gain=1.5, pct=(0.5, 99.5)):
    """
    Restore high-frequency detail from `raw` into `warped`, inside `mask`

    param warped (ndarray): Motion-corrected data (3D or 4D)
    param raw (ndarray): Original data
    param mask (ndarray): Binary (3D: X,Y,Z)
    param sigmas (tuple): 2D smoothing sigmas for (x,y) in voxel units
    param gain (float): Texture gain factor.
    param pct (tuple): (low, high) percentiles for clipping, computed from raw within mask.

    return restored data (ndarray): np.ndarray , same shape as input.
    """
    ...
    # Validate shapes
    if warped.ndim != 4 or raw.ndim != 4:
        raise ValueError(f"warped and raw must be 4D (X,Y,Z,T). Got warped={warped.shape}, raw={raw.shape}")
    if warped.shape != raw.shape:
        raise ValueError(f"warped and raw must have same shape. Got warped={warped.shape}, raw={raw.shape}")
    if mask.ndim != 3:
        raise ValueError(f"mask must be 3D (X,Y,Z). Got mask={mask.shape}")
    if warped.shape[:3] != mask.shape:
        raise ValueError(f"mask spatial shape must match images. Got mask={mask.shape}, image={warped.shape[:3]}")

    warped = warped.astype(np.float32, copy=False)
    raw = raw.astype(np.float32, copy=False)

    X, Y, Z, T = warped.shape
    out = np.empty_like(warped, dtype=np.float32)
    ...
    for t in range(T):
        for z in range(Z):
            img_warped = warped[:, :, z, t]
            img_raw = raw[:, :, z, t]
            mask_slice = mask_bool[:, :, z]

            if not np.any(mask_slice):
                out[:, :, z, t] = img_warped
                continue

            raw_smooth = smooth(img_raw, sigmas=list(sigmas))
            texture = img_raw - raw_smooth
            restored = img_warped + (gain * texture)

            vals = img_raw[mask_slice]
            lo, hi = np.percentile(vals, [lo_p, hi_p])

            out_slice = img_warped.copy()
            out_slice[mask_slice] = np.clip(restored[mask_slice], lo, hi)
            out[:, :, z, t] = out_slice
