...
class SCTArgumentParser(argparse.ArgumentParser):
    ...
    @cached_property
    def default_arg_group(self):
        """
        A default argument group (named "MISC") that arguments generated by the utility functions of this class
        will be placed in if no other group is specified.

        This is a cached property to prevent it being generated more than once after initially needed (which would
        delete any prior arguments placed within it; no idea why that is the default ArgParse behaviour)
        """
        return self.add_argument_group("MISC")
    ...
    def add_common_args(self, arg_group=None):
        ...
        If no argument group is provided, the arguments are placed into the "OPTIONAL" argument group;
        in either case, the argument group the arguments were inserted into is returned for reference
        """
        # If the user didn't specify an argument group,
        if not arg_group:
            arg_group = self.default_arg_group
