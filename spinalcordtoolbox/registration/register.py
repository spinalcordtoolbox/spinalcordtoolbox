...
from spinalcordtoolbox.utils import sct_progress_bar, copy_helper, run_proc, tmp_create, get_interpolation
...
def slicereg_crop_extents(src, dest):
    """
    Find the min (and max) z-slice index below which (and above which) slices only have voxels below a given threshold.
    """
    # TODO: Double-check whether this step is even necessary. This was preserved as-is during a refactor of the
    #       function `register_step_ants_slice_regularized_registration`.
...
def antsSliceRegularizedRegistration(fname_fixed_image, fname_moving_image,
    ...
    warp_forward_out = f'{output_prefix}Warp.nii.gz'
    warp_inverse_out = f'{output_prefix}InverseWarp.nii.gz'
    txty_csv_out = f'{output_prefix}TxTy_poly.csv'
