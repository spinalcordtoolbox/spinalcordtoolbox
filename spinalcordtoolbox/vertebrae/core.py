...
def vertebral_detection(fname, fname_seg, contrast, param, init_disc, verbose=1, path_template='', path_output='../',
    ...
    if contrast == "t2":
        sct_deepseg.main(['-i', 'input_image.nii.gz', '-task', 'find_disc_t2', '-thr', 0.5, '-o', 'hm_tmp.nii.gz'])
    elif contrast == "t1":
        sct_deepseg.main(['-i', 'input_image.nii.gz', '-task', 'find_disc_t1', '-thr', 0.5, '-o', 'hm_tmp.nii.gz'])
