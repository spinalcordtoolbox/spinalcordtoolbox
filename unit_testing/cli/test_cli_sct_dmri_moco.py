...
# TODO: Replace calls to CLI scripts with API equivalents
...
def test_sct_dmri_moco_with_mask_check_params(tmp_path, dmri_mask):
    ...
    df = read_csv(tmp_path / "moco_params.tsv", sep="\t")
    lresults = list(df['X'][:])
    lgroundtruth = [0.008491259664160821, 0.0029020670607778237, 0.0029020670607778237, 0.0029020670607778237,
                    -0.014405996135095476, -0.014405996135095476, -0.014405996135095476]
    assert allclose(lresults, lgroundtruth)
