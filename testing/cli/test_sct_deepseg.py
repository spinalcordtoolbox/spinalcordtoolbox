...
def test_load_crop_metadata(tmp_path, monkeypatch):
    ...
    explicit, complete `crop_metadata.yaml` for crop models on official installs; tolerate a
    missing file only for `-custom-url` installs; and reject a yaml that's missing any of the
    padding keys, rather than silently falling back to `sc_crop.detect()`'s own defaults.
    """
    monkeypatch.setitem(spinalcordtoolbox.deepseg.models.MODELS, 'not_a_crop_model', {'cropped_image': False})
    monkeypatch.setitem(spinalcordtoolbox.deepseg.models.MODELS, 'a_crop_model', {'cropped_image': True})
    path_model = str(tmp_path)

    # Non-crop model: nothing on disk is even checked.
    assert spinalcordtoolbox.deepseg.models.load_crop_metadata('not_a_crop_model', path_model) == {}

    # Crop model, no crop_metadata.yaml, official install: raises.
    with pytest.raises(RuntimeError) as e:
        spinalcordtoolbox.deepseg.models.load_crop_metadata('a_crop_model', path_model)
    assert "no crop_metadata.yaml was found" in str(e.value)

    # Crop model, no crop_metadata.yaml, `-custom-url` install: tolerated.
    fname_source = tmp_path / "source.json"
    fname_source.write_text(json.dumps({'custom': True}))
    assert spinalcordtoolbox.deepseg.models.load_crop_metadata('a_crop_model', path_model) == {}
    fname_source.unlink()

    # crop_metadata.yaml present but missing some padding keys: raises.
    fname_yaml = tmp_path / "crop_metadata.yaml"
    fname_yaml.write_text("pad_superior: 40.0\n")
    with pytest.raises(RuntimeError) as e:
        spinalcordtoolbox.deepseg.models.load_crop_metadata('a_crop_model', path_model)
    assert "missing padding key" in str(e.value)

    # crop_metadata.yaml present, all padding keys given: returned as-is.
    full_pad = {key: float(i) for i, key in enumerate(spinalcordtoolbox.deepseg.models.CROP_PAD_KEYS)}
    fname_yaml.write_text("\n".join(f"{k}: {v}" for k, v in full_pad.items()))
    assert spinalcordtoolbox.deepseg.models.load_crop_metadata('a_crop_model', path_model) == full_pad
