...
def init(param_test):
    ...
    try:
        ...
        nib.save(img, param_test.full_path_tmp_data + 'data_seg.nii.gz')
        ...
        nib.save(img_rothd, param_test.full_path_tmp_data + 'data_seg_rothd.nii.gz')

        # rotate src image
        sct.run('sct_label_utils -i ' + param_test.full_path_tmp_data + 'data_seg.nii.gz -create 13,13,0,3:13,13,15,2:13,16,15,4:13,13,30,1 -o ' + param_test.full_path_tmp_data + 'data_rot_src.nii.gz', 0)
        sct.run('sct_label_utils -i ' + param_test.full_path_tmp_data + 'data_seg.nii.gz -create 4,13,6,3:13,13,15,2:13,16,15,4:22,13,26,1 -o ' + param_test.full_path_tmp_data + 'data_rot_dest.nii.gz', 0)
        sct.run('sct_register_multimodal -i ' + param_test.full_path_tmp_data + 'data_seg.nii.gz -d ' + param_test.full_path_tmp_data + 'data_seg.nii.gz -ilabel ' + param_test.full_path_tmp_data + 'data_rot_src.nii.gz -dlabel ' + param_test.full_path_tmp_data + 'data_rot_dest.nii.gz -param step=0,type=label,dof=Rx_Ry:step=1,type=im,algo=syn,iter=0 -x linear -ofolder ' + param_test.full_path_tmp_data, 0)
...
def test_integrity(param_test):
    ...
    if index_args == 0:  # only do this integrity test once
        try:
            ...
            # compute CSA
            cmd = 'sct_process_segmentation -i ' + param_test.full_path_tmp_data + 'data_seg.nii.gz' \
                  + ' -p csa' \
                  + ' -size 0' \
                  + ' -r 0' \
                  + ' -v 1' \
            ...
            cmd = 'sct_process_segmentation -i ' + param_test.full_path_tmp_data + 'data_seg_src_reg.nii.gz' \
                  + ' -p csa' \
                  + ' -size 0' \
                  + ' -r 0' \
                  + ' -v 1' \
            ...
            cmd = 'sct_process_segmentation -i ' + param_test.full_path_tmp_data + 'data_seg_rothd.nii.gz' \
                  + ' -p csa' \
                  + ' -size 0' \
                  + ' -r 0' \
                  + ' -v 1' \
