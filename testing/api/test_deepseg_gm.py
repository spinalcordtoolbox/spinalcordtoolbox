...
import spinalcordtoolbox.deepseg_.gm as deepseg_gm


class TestCore(object):
    def test_data_resource(self):
        """Test the DataResource manager, and check if files exists."""
        resource_models = deepseg_gm.DataResource('deepseg_gm_models')
        for model_name in deepseg_gm.MODELS.keys():
            model_path, metadata_path = deepseg_gm.MODELS[model_name]
    ...
    def test_shape_transformation(self):
        ...
        for dim_old in ((200, 200), (203, 202), (198, 197)):
            ...
            shape_transform = deepseg_gm.ShapeTransform(dummy_img.shape, dim_new)
    ...
    def test_thresholding(self):
        ...
        thr_ret = deepseg_gm.threshold_predictions(dummy_preds, 0.5)
        ...
        dummy_preds = np.full((200, 200), 0.4)
        thr_ret = deepseg_gm.threshold_predictions(dummy_preds, None)
    ...
    def test_segment_volume(self):
        ...
        for dim in ((200, 200, 3), (203, 202, 2), (198, 197, 1)):
            ...
            ret = deepseg_gm.segment_volume(img, 'challenge')
    ...
    def test_standardization_transform(self):
        ...
        np_data[..., 0] = 5.0
        transform = deepseg_gm.StandardizationTransform(3.0, 2.0)
    ...
    def test_volume_standardization_transform(self):
        ...
        np_data = np.ones((200, 200, 2), dtype=np.float32)
        np_data[..., 0] = 5.0
        transform = deepseg_gm.VolumeStandardizationTransform()
        np_transformed_data = transform(np_data)
        assert np_transformed_data.mean() == 0.0
        assert np_transformed_data.std() == 1.0
